...
import ***REMOVED***.util.DataFrameClasses.{ Address, Person }
import ***REMOVED***.util.HDFSBase
import play.Logger
import play.api.Application
import play.api.***REMOVED***.Status
  ...
  "The storage_manager" should "manage the physical datasets correctly" in new WithServer(app = application, port = getAvailablePort) {
    ...
    WsTestClient.withClient { implicit client =>
      ...
      val response: WSResponse = Await.result[WSResponse](client.
        url(s"***REMOVED***://localhost:$port/storage-manager/v1/physical-datasets")
    ...
    WsTestClient.withClient { implicit client =>
      ...
      val response: WSResponse = Await.result[WSResponse](client.
        url(s"***REMOVED***://localhost:$port/storage-manager/v1/physical-datasets/schema").
        withAuth("david", "david", WSAuthScheme.BASIC).
        post(json), Duration.Inf)

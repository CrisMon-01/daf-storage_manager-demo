...
import ***REMOVED***.catalogmanager.CatalogManagerClient
import ***REMOVED***.dataset.export.FileExportService
import ***REMOVED***.dataset._
import ***REMOVED***.dataset.query.jdbc.JdbcQueryService
import ***REMOVED***.dataset.query.Query
import ***REMOVED***.dataset.query.json.QueryFormats.reader
import ***REMOVED***.error.InvalidRequestException
import ***REMOVED***.instances.{ FileSystemInstance, ImpalaTransactorInstance }
import ***REMOVED***.web._
import ***REMOVED***.filesystem.{ DownloadableFormats, FileDataFormat }
import it.gov.***REMOVED***.common.config.{ ConfigReadException, Read }
...
class DatasetController @Inject()(configuration: Configuration,
  ...
  private def defaultLimit = Read.int { "***REMOVED***.row_limit" }.read(configuration) getOrElse None

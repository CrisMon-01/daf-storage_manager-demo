...
import ***REMOVED***.dataset.{ DatasetParams, FileDatasetParams, KuduDatasetParams }
import ***REMOVED***.filesystem.fileFormatShow
...
object PhysicalDatasetController {
  ...
  def apply(configuration: Config): PhysicalDatasetController = {
    ...
    val defaultLimit = if (configuration hasPath "***REMOVED***.row_limit") Some {
      configuration.getInt("***REMOVED***.row_limit")
    } else None

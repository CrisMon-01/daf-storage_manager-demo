...
import scalaj.***REMOVED***.{ Http, HttpResponse }

import scala.util.{ Failure, Try, Success => TrySuccess }

class CatalogManagerClient(serviceUrl: String) {

  val logger = LoggerFactory.getLogger("it.gov.***REMOVED***.CatalogManager")
...
object CatalogManagerClient {
  ...
  def fromConfig(config: Configuration) = Read.string { "***REMOVED***.catalog_url" }.!.read(config) match {

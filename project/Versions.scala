object Versions {
  ...
  lazy val defaultVersion = sys.***REMOVED***.getOrElse("STORAGE_MANAGER_VERSION", "1.0.2-SNAPSHOT")

  lazy val isSnapshot = Versions.defaultVersion.endsWith("SNAPSHOT")

  // internal

  lazy val common = sys.***REMOVED***.getOrElse("COMMON_VERSION", "1.0.9-SNAPSHOT")

...
object Docker {
  ...
  val repository: Option[String] = Versions.choose(
    whenSnapshot = Some { "nexus.***REMOVED***.***REMOVED***.test"   },
    whenRelease  = Some { "nexus.***REMOVED***.***REMOVED***.it" }
  )
  ...
  private val addConfiguration = Seq(
    Cmd("COPY", "***REMOVED***.conf",         "conf/***REMOVED***.conf"),
  ...
  def entryPoint(artifactName: String) = Seq(s"bin/$artifactName", s"-Dconfig.file=conf/app/***REMOVED***.conf")
}
